...
import util.renderers.RenderTask;
...
public class HealthBarSegment {
    ...
    public static void printOnScreen(Renderer renderer, Vector2 position, double hpRatio, Color color) {
        ...
        renderer1.drawRectangle2d(bgColor, position.toAwtPoint(), bgPieceSize.x, bgPieceSize.y, true);
        namedRenderer1.finishAndSend();
        NamedRenderer namedRenderer2 = new NamedRenderer("Group2");
        namedRenderer2.startPacket();
        renderer1.drawRectangle2d(bgColor, position.plus(offsetAB).toAwtPoint(), bgPieceSize.x, bgPieceSize.y, true);
        namedRenderer2.finishAndSend();

        NamedRenderer namedRenderer3 = new NamedRenderer("Group3");
        namedRenderer3.startPacket();
        renderer1.drawRectangle2d(color, position.plus(offsetCD).toAwtPoint(), coloredHealthPieceSize.x, coloredHealthPieceSize.y, true);
        namedRenderer3.finishAndSend();
        NamedRenderer namedRenderer4 = new NamedRenderer("Group4");
        namedRenderer4.startPacket();
        renderer1.drawRectangle2d(color, position.plus(offsetAB).plus(offsetCD).toAwtPoint(), coloredHealthPieceSize.x, coloredHealthPieceSize.y, true);
        namedRenderer4.finishAndSend();
    }
}
